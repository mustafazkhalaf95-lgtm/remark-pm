...
import { creativeRequestCreateSchema } from '@/lib/validations';
...
export async function GET(req: Request) {
    ...
    try {
        ...
        const filters = parseFilters(req.url, ['clientId', 'status', 'priority', 'category', 'assignedTo', 'campaignId']);
        const search = new URL(req.url).searchParams.get('search') || undefined;

        const where = buildWhere(filters, ['title', 'titleAr', 'brief'], search);
...
export async function POST(req: Request) {
    ...
    try {
        const data = await validateBody(req, creativeRequestCreateSchema);
        ...
        await logAudit(auth.session.user.id, 'created', 'creative_requests', { id: item.id, title: item.title });
